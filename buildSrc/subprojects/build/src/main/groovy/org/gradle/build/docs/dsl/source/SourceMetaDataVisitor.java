...
public class SourceMetaDataVisitor extends VoidVisitorAdapter<ClassMetaDataRepository<ClassMetaData>> {
        ...
        for (ImportDeclaration anImport : compilationUnit.getImports()) {
            if (!anImport.isStatic()) {
                for (ClassMetaData classMetaData : allClasses) {
                    classMetaData.addImport(anImport.getNameAsString() + (anImport.isAsterisk() ? ".*" : ""));
                }
            }
        }
    }
    ...
    private void visitTypeDeclaration(TypeDeclaration<?> typeDeclaration, ClassMetaDataRepository<ClassMetaData> repository, ClassMetaData.MetaType metaType, Runnable action) {
        ...
        String comment = typeDeclaration.getJavadocComment().map(Comment::getContent).orElse("");
    ...
    public void visit(MethodDeclaration methodDeclaration, ClassMetaDataRepository<ClassMetaData> repository) {
        ...
        String rawCommentText = methodDeclaration.getJavadocComment().map(Comment::getContent).orElse("");
        ...
        if (matcher.matches()) {
            ...
            for (String annotation : methodMetaData.getAnnotationTypeNames()) {
    ...
    private TypeMetaData extractTypeName(Type type) {
        ...
        type.ifArrayType(arrayType -> {
            for (int i = 0; i < arrayType.getArrayLevel(); i++) {
                typeMetaData.addArrayDimension();
            }
        });
        ...
        elementType.ifWildcardType(wildcardType -> {
            if (!wildcardType.getExtendedType().isPresent() && !wildcardType.getSuperType().isPresent()) {
                ...
                wildcardType.getExtendedType().ifPresent(referenceType -> {
                    referenceType.ifClassOrInterfaceType(classOrInterfaceType -> typeMetaData.setUpperBounds(extractTypeName(classOrInterfaceType)));
                });
                wildcardType.getSuperType().ifPresent(referenceType -> {
                    referenceType.ifClassOrInterfaceType(classOrInterfaceType -> typeMetaData.setLowerBounds(extractTypeName(classOrInterfaceType)));
                });
            }
        });
